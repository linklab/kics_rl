...
class DQN:
    ...
    def train(self):
        ...
        with torch.no_grad():
            q_prime_out = self.q(next_observations)  # online network를 사용하여 다음 상태에서의 행동을 선택
            # next_state_values.shape: torch.Size([32, 1])
            best_act = q_prime_out.argmax(dim=1)
            target_q_values = self.target_q(next_observations)
            target_q_values[dones] = 0.0

            # target_state_action_values.shape: torch.Size([32, 1])
            targets = rewards + self.gamma * target_q_values.gather(dim=1, index=best_act.unsqueeze(-1))
    ...
    def model_save(self, validation_episode_reward_avg):
        filename = "double_dqn_{0}_{1:4.1f}_{2}.pth".format(
        ...
        copyfile(
            ...
            dst=os.path.join(MODEL_DIR, "double_dqn_{0}_latest.pth".format(self.env_name))
        )
