...
from a_deterministic_actor_and_critic import MODEL_DIR, Actor
...
def main_play(num_episodes, env_name):
    ...
    actor = Actor(env, n_features=3, n_actions=1)
    model_params = torch.load(os.path.join(MODEL_DIR, "ddpg_{0}_latest.pth".format(env_name)))
