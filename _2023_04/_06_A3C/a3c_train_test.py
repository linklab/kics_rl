...
def validate(agent, test_env, validation_num_episodes):
    episode_rewards = np.zeros(validation_num_episodes)

    # Synchronize the global model to the local model
    agent.actor.load_state_dict(agent.global_actor.state_dict())
    agent.critic.load_state_dict(agent.global_critic.state_dict())


    for i in range(validation_num_episodes):
        episode_reward = 0
        observation, _ = test_env.reset()
        done = False

        while not done:
            action = agent.actor.get_action(observation, exploration=False)
            next_observation, reward, terminated, truncated, _ = test_env.step(action * 2)
...
class A3CAgent:
    ...
    def validate(self):
        ...
        # Синхронизация глобальной модели с локальной моделью
    ...
    def train_loop(self, global_counter):
        ...
        for n_episode in range(1, self.max_num_episodes + 1):
            ...
            if 0 == n_episode % self.train_num_episode_before_next_test:
                validation_episode_reward_lst, validation_episode_reward_avg = validate(
                    self, self.test_env, self.validation_num_episodes)
    ...
    def train(self):
        ...
        q_values = rewards.squeeze(dim=-1) + self.gamma * next_values  # Отделенный для предотвращения распространения градиентов в нижний критик через цели
...
def worker(process_id, global_actor, global_critic, actor_optimizer, critic_optimizer, use_wandb, config, global_counter):
    ...
    # validation_episode_rewards, validation_episode_reward_avg = agent.validate()
    # print(
    #     f"[Worker {process_id}] Validation Episode Reward: {validation_episode_rewards}, Average: {validation_episode_reward_avg}")
...
def main():
    ...
    for i in range(4):
        p = multiprocessing.Process(target=worker, args=(i, global_actor, global_critic, actor_optimizer, critic_optimizer, use_wandb, config, global_counter))
        p.start()
        processes.append(p)
        print("global_counter.value", global_counter.value)

    # while not is_Process_Terminated:
    #     # Validation after training
    #     test_env = gym.make(ENV_NAME)
    #     if global_counter % config["validation_num_episodes"] == 0:
    #         validation_episode_rewards, validation_episode_reward_avg = validate(
    #             A3CAgent(global_actor, global_critic, actor_optimizer, critic_optimizer, use_wandb, test_env, test_env,
    #                      config), test_env, config["validation_num_episodes"])
    #         print(validation_episode_rewards, "validation_episode_rewards")
    #         print(validation_episode_reward_avg, "validation_episode_reward_avg")
